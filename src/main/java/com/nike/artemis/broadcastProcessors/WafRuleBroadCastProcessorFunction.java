...
public class WafRuleBroadCastProcessorFunction extends BroadcastProcessFunction<WafRequestEvent, WafRuleChange, Tuple4<String, WafRateRule, Long, String>> {
    ...
    public void processElement(WafRequestEvent wafRequestEvent, BroadcastProcessFunction<WafRequestEvent, WafRuleChange,
            Tuple4<String, WafRateRule, Long, String>>.ReadOnlyContext ctx, Collector<Tuple4<String, WafRateRule, Long, String>> out) throws Exception {
        for (Map.Entry<WafRateRule, Object> entry : ctx.getBroadcastState(wafRulesStateDescriptor).immutableEntries()) {
            if (entry.getKey().appliesTo(wafRequestEvent)) {
                ...
                out.collect(new Tuple4<>(wafRequestEvent.getUser(), entry.getKey(), wafRequestEvent.getTime(), wafRequestEvent.getUpmid()));
            }
        }
    }
    ...
    public void processBroadcastElement(WafRuleChange wafRuleChange, BroadcastProcessFunction<WafRequestEvent, WafRuleChange,
            Tuple4<String, WafRateRule, Long, String>>.Context ctx, Collector<Tuple4<String, WafRateRule, Long, String>> out) throws Exception {
