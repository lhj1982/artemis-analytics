...
public class CdnLogResolverTest {
    ...
    @Test
    public void test_dontPass_whenUpmidWithSwooshUser() {
        ...
        String payload = "{\"prn\":\"test-upmid\",\"prt\":\"nike:swoosh\"}";
    ...
    @Test
    public void test_dontPass_whenUpmidWithAthleteUser() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
        String payload = "{\"prn\":\"test-upmid\",\"prt\":\"nike:athlete\"}";
        Base64.Encoder encoder = Base64.getUrlEncoder();
        String jwtToken = "auth=Bearer start." + encoder.encodeToString(payload.getBytes()) + ".end";
        String cdnLog = "{\n" +
                "\t\"unixtime\": \"1694653776\",\n" +
    ...
    @Test
    public void test_pass_whenUpmidWithPlusUser() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
        String payload = "{\"prn\":\"test-upmid\",\"prt\":\"nike:plus\"}";
        Base64.Encoder encoder = Base64.getUrlEncoder();
        String jwtToken = "auth=Bearer start." + encoder.encodeToString(payload.getBytes()) + ".end";
        String cdnLog = "{\n" +
                "\t\"unixtime\": \"1694653776\",\n" +
    ...
    @Test
    public void test_pass_whenIpAddressUser() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
        String cdnLog = "{\n" +
                "\t\"unixtime\": \"1694653776\",\n" +
                "\t\"uuid\": \"6a0fda9616946537764337109e\",\n" +
                "\t\"user_info\": \"accEnc=br;q=1.0, gzip;q=0.9, deflate;q=0.8||accLang=zh-Hans-CN;q=1.0, wuu-Hans-CN;q=0.9, en-IN;q=0.8||accRange=-||age=-||allow=GET,POST,OPTIONS,PUT,DELETE||allowOrigin=*||cacheCtl=-||conn=-||contDisp=-||contEnc=-||contLang=-||contMD5=-||date=Thu, 14 Sep 2023 01:09:36 GMT||DNT=-||edgeControl=-||eTag=-||expect=-||expires=-||fwdHost=-||ifMatch=-||ifMod=-||ifNone=-||ifRange=-||ifUnmod=-||lastMod=-||link=-||p3p=-||parentSpanId=-||protoVer=HTTP/2.0||redirURL=-||respCacheCtl=-||respConn=close||respContMD5=-||respCT=application/json;charset=UTF-8||retry=-||sampleable=-||server=-||set_cookie=-||spanId=-||spanName=-||te=-||traceId_in=feff33e0c5fe82b8||traceId_out=3bdeb671258bb45e||trailer=-||transEnc=-||upgrade=-||vary=-||warning=-||wwwAuth=-||xPwrdBy=-||xReqWith=-||zhenghe_count=578044||zhenghe_githash=-||zhenghe_target_host=auth.prod.commerce.origin.nike.com.cn||zhenghe_timing_ms=12||zhenghe_version=v6.10.1||zhenghe_bm_result=-||ip_region_en=Shanghai||ip_country_en=China||ip_city_en=Shanghai\",\n" +
                "\t\"client_ip\": \"220.196.194.29\",\n" +
                "\t\"user_agent\": \"NikeApp/23.42.1 (prod; 2308312223; iOS 16.6.1; iPhone14,3)\",\n" +
                "\t\"method\": \"PUT\",\n" +
                "\t\"return_code\": \"202\",\n" +
    ...
    @Test
    public void test_parseValidCdnLogUpmid() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
        String payload = "{\"prn\":\"test-upmid\",\"prt\":\"nike:plus\"}";
        ...
        cdnLogResolver.flatMap(cdnLog, collector);
        Assert.assertEquals("test-upmid", out.get(0).getUser());
        Assert.assertEquals(CdnUserType.upmid.name(), out.get(0).getUserType());
    }

    @Test
    public void test_parseValidCdnLogIp() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
        String cdnLog = "{\n" +
        ...
        cdnLogResolver.flatMap(cdnLog, collector);
        Assert.assertEquals("220.196.194.29", out.get(0).getUser());
        Assert.assertEquals(CdnUserType.ipaddress.name(), out.get(0).getUserType());
    }

    @Test
    public void test_doesNotFail_whenEmptyCdnLog() {
        CdnLogResolver cdnLogResolver = new CdnLogResolver();
        List<CdnRequestEvent> out = new ArrayList<>();
        ListCollector<CdnRequestEvent> collector = new ListCollector<>(out);
